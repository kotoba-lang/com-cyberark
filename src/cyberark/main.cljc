(ns cyberark.main
  "Kotodama WASM entrypoint for the Cyberark clean-room actor (L4) — Clojure port.
  ...
  facts (`cyberark.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cyberark")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"cyberark.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cyberark-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cyberark-compat/ready)
